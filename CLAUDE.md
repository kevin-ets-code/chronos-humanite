...
- **Lift gamma océans** — `uOceanGamma` (défaut 0.85) : lift gamma appliqué aux océans uniquement (la bathymétrie Blue Marble ressort trop sombre), masqué par la specular map (`specMask`, hissé là puis réutilisé par le sun glint), appliqué après le decode pow 2.2 et avant terminateur / mix jour-nuit (crépuscule et nuit restent cohérents).
